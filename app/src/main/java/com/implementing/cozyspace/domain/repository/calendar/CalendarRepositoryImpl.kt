...
import com.implementing.cozyspace.util.extractEndFromDuration
import com.implementing.cozyspace.util.extractFrequency
import com.implementing.cozyspace.util.getEventDuration
import com.implementing.cozyspace.util.getEventRule
        ...
        return withContext(Dispatchers.IO) {
            ...
            if (cur != null) {
                while (cur.moveToNext()) {
                    ...
                    val end: Long =
                        if (duration.isNotBlank()) duration.extractEndFromDuration(start) else cur.getLong(
                            END_INDEX
                        )
...
//                    val frequency: String = rrule.extractFrequency()
            ...
            if (curI != null) {
                while (curI.moveToNext()) {
                    ...
                    val end: Long =
                        if (duration.isNotBlank()) duration.extractEndFromDuration(start) else curI.getLong(
                            END_INDEX
                        )
    ...
    override suspend fun addEvent(event: CalendarEvent) {
        withContext(Dispatchers.IO) {
            val values = ContentValues().apply {
                ...
                if (event.recurring) {
                    put(CalendarContract.Events.RRULE, event.getEventRule())
